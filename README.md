...
- **`cmnlib::check_file_checksum`**\
  Computes the checksum of a file and checks that it matches the one
...
- **`cmnlib::download`**\
  Downloads the file pointed by the given URL and stores it at the
  given path.
...
- **`cmnlib::list_env_vars`**\
  ...
  A few environment variables are ignored: `PATH`, `GIT_DIR`, `CPATH`,
  `CPPATH`, `LD_PRELOAD`, `LIBRARY_PATH`, `LD_LIBRARY_PATH`, `JAVA_OPTS`,
